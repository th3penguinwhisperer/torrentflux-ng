		...
		if (is_integer(strpos($thing,"Your search did not match any torrents")))
		{
			...
			if (strpos($thing,"searchResult\"") !== false)
			{
				...
				foreach($tmpListArr as $key => $value)
				{
					...
					if (strpos($value,"static.thepiratebay.se"))
					{
...
class pBay
{
	...
	function BuildOutput($bg, $searchURL='', $maxDisplayLength=80)
	{
		...
		$output = "<tr>\n";
		$output .= "	<td width=\"16\" bgcolor=\"".$bg."\"><a href=\"javascript:addTransfer(escape('".$this->torrentFile."'));\"><img src=\"".getImagesPath()."download_owner.gif\" width=\"16\" height=\"16\" title=\"".$this->torrentName."\" border=0></a></td>\n";
		$output .= "	<td bgcolor=\"".$bg."\"><a href=\"javascript:addTransfer(escape('".$this->torrentFile."'));\" title=\"".$this->torrentName."\">".$this->torrentDisplayName."</a></td>\n"; // USE ESCAPE FUNCTION OTHERWHISE TITLE IS NOT SHOWN BEFORE THE TORRENT IS DOWNLOADED, ONLY THE ID IS SHOWN THEN
