...
if (!empty($testpostgres)) {
	...
	if ($db->Connect("hostnameOrIp","tester","test","test")) {
...
if (!empty($testibase)) {
	...
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($db->PConnect("hostnameOrIp:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", ""))
...
if (!empty($testaccess)) {
	...
	print "<h1>Connecting $db->databaseType...</h1>";
	$access = 'd:\inetpub\wwwusername\php\NWIND.MDB';
	$dsn = "nwind";
...
if (!empty($testvfp)) { // ODBC
	...
	if ( $db->PConnect('hostnameOrIp','DRIVER={Microsoft Visual FoxPro Driver};SOURCETYPE=DBF;SOURCEDB=d:\inetpub\adodb;EXCLUSIVE=NO;')) {
	print "<h1>Connecting $db->databaseType...</h1>";flush();
	testdb($db,"create table d:\\inetpub\\adodb\\ADOXYZ (id int, firstname char(24), lastname char(24),created date)");
...
// REQUIRES MySQL server at hostnameOrIp with database 'test'
if (!empty($testmysql)) { // MYSQL


	if (PHP_VERSION >= 5 || $_SERVER['HTTP_HOST'] == 'hostnameOrIp') $server = 'hostnameOrIp';
	else $server = "mangrove";
	$user = 'username'; $password = ''; $database = 'northwind';
	$db = ADONewConnection("mysqlt://$user:$password@$server/$database?persist");
	print "<h1>Connecting $db->databaseType...</h1>";
	
	if (true || $db->PConnect($server, "username", "", "northwind")) {
		//$db->Execute("DROP TABLE ADOXYZ") || die('fail drop');
		//$db->debug=1;$db->Execute('drop table ADOXYZ');
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) Type=InnoDB");
	} else print "ERROR: MySQL test requires a MySQL server on hostnameOrIp, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
}
...
if (!empty($testmysqli)) { // MYSQL
	...
	if ($db->PConnect($server, "username", "", "northwind")) {
		//$db->debug=1;$db->Execute('drop table ADOXYZ');
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date)");
	} else print "ERROR: MySQL test requires a MySQL server on hostnameOrIp, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testmysqlodbc)) { // MYSQL
	...
	$db->hasTransactions = false;
	print "<h1>Connecting $db->databaseType...</h1>";
	if ($_SERVER['HTTP_HOST'] == 'hostnameOrIp') $server = 'hostnameOrIp';
	else $server = "mangrove";
	if ($db->PConnect('mysql', "username", ""))
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb");
	else print "ERROR: MySQL test requires a MySQL server on hostnameOrIp, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
}
...
if (!empty($testproxy)){
	...
	if ($db->PConnect('http://hostnameOrIp/php/phplens/adodb/server.php'))
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb");
	else print "ERROR: MySQL test requires a MySQL server on hostnameOrIp, userid='admin', password='', database='test'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testdb2)) {
	if (PHP_VERSION>=5.1) {
		...
		#$dsn = "driver={IBM db2 odbc DRIVER};Database=test;hostname=hostnameOrIp;port=50000;protocol=TCPIP; uid=natsoft; pwd=guest";
		if ($db->Connect('hostnameOrIp','natsoft','guest','test')) {
			testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
...
echo "<hr />";
flush();
	$dsn = "driver={IBM db2 odbc DRIVER};Database=sample;hostname=hostnameOrIp;port=50000;protocol=TCPIP; uid=username; pwd=natsoft";
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
	...
	//$db->debug=1;
	$myDSN="SERVER=hostnameOrIp;DATABASE=northwind;Trusted_Connection=yes";
...
if (extension_loaded('odbtp') && !empty($testmssql)) { // MS SQL Server via ODBC
	...
	if ($db->PConnect('hostnameOrIp',$dsn, "", ""))  {
		print "<h1>Connecting $db->databaseType...</h1>";				
		testdb($db,"create table ADOXYZ (id int, firstname char(24) null, lastname char(24) null,created datetime null)");
	}
	else print "ERROR: MSSQL test 1 requires a MS SQL 7 server setup with DSN setup";
