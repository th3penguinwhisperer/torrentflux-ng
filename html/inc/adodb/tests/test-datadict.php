...
";
	...
	if (file_exists('d:\inetpub\wwwusername\php\phplens\adodb\adodb.inc.php'))
	if ($dbType == 'mysqlt') {
		$db->Connect('hostnameOrIp', "username", "", "test");
		$dict->SetSchema('');
	...
	if ($dbType == 'odbc_mssql') {
		$dsn = $dsn = "PROVIDER=MSDASQL;Driver={SQL Server};Server=hostnameOrIp;Database=northwind;";
