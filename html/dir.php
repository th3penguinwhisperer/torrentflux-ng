...
// TODO write code to check if the path being viewed should be accessible by the user
...
if ($chmod != "") {
	...
	@header("Location: dir.php&dir=".UrlHTMLSlashesEncode($dir));
	exit();
}
...
// check dir-var
//$dir = $dirName; // TODO set to what is sent in the request
if (isset($dir)) {
	if ($dir != "" && substr($dir,strlen($dir)-1,1)!="/") {
		$dir = $dir."/";
	}
...
if (($dir != "") && (hasPermission($dir, $cfg["user"], 'r') !== true)) {
	AuditAction($cfg["constants"]["error"], "ILLEGAL DIR: ".$cfg["user"]." tried to access ".$dir);
	@error("No Permission for Dir", "dir.php", "", array($dir));
}
...
while (false !== ($entry = readdir($handle))) {
	if (empty($dir)) { // parent dir
		// TODO get this back to checking if access to a directory should be allowed
...
{
	if (preg_match("/^(.+)\/.+$/", $dir, $matches) == 1)
		$tmpl->setvar('parentURL', "dir.php?dir=" . UrlHTMLSlashesEncode($matches[1]));
	else
		$tmpl->setvar('parentURL', "dir.php");
